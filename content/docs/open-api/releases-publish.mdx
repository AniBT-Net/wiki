...
<TypeTable
  type={{
    'animeId': {
      description: '站内番剧 ID。一般不需要手填；更常用 bgmId。若 animeId 与 bgmId 同时传入，优先按 animeId 查找。',
      type: 'string',
    },
    'bgmId': {
      description: 'Bangumi 番剧 ID。可通过 /api/bgm/search 查询。未传时服务端会尝试按种子标题自动匹配，失败则进入待匹配状态。',
      type: 'number',
    },
    'malId': {
      description: 'MyAnimeList anime id。按 bangumi-data 与 BGM 的对应关系解析；若同一个 MAL ID 对应多个 BGM 条目且无法唯一确定，会进入待匹配状态。',
      type: 'number',
    },
    'anidbId': {
      description: 'AniDB anime id。按 bangumi-data 与 BGM 的对应关系解析；若同一个 AniDB ID 对应多个 BGM 条目且无法唯一确定，会进入待匹配状态。',
      type: 'number',
    },
    'anilistId': {
      description: 'AniList anime id。按 bangumi-data 与 BGM 的对应关系解析；若同一个 AniList ID 对应多个 BGM 条目且无法唯一确定，会进入待匹配状态。',
      type: 'number',
    },
    'title': {
...
| 字段 | 可填值 | 说明 |
| ---- | ------ | ---- |
| `bgmId` | Bangumi subject id，例如 `400602` | 推荐填写。可先调用 [`/api/bgm/search`](/docs/open-api/bgm-search) 搜索番剧。 |
| `malId` | MyAnimeList anime id，例如 `52991` | 可替代 `bgmId`。本站按 bangumi-data 的 BGM 对应关系解析。 |
| `anidbId` | AniDB anime id，例如 `17617` | 可替代 `bgmId`。同一个 AniDB ID 对应多个 BGM 条目时会进入待匹配状态。 |
| `anilistId` | AniList anime id，例如 `154587` | 可替代 `bgmId`。同一个 AniList ID 对应多个 BGM 条目时会进入待匹配状态。 |
| `animeId` | 站内 `anime_...` / Convex doc id 字符串 | 高级用法。一般不要手填，除非你的系统已经保存了站内番剧 ID。 |
...
番剧识别优先级为 `animeId` / `bgmId`，然后是 `malId` / `anidbId` / `anilistId`，最后才按标题和种子内部文件名自动匹配。外部 ID 的对应关系以 bangumi-data 为准；若外部 ID 对应多个 BGM 条目或多个外部 ID 解析结果互相冲突，资源会进入待匹配状态，之后由字幕组或管理员补匹配。
