...
如果自动化只有种子标题、还没有稳定番剧 ID，可先调用
[`/api/animes/torrent-match`](/docs/open-api/anime-torrent-match)。只有响应里的
`shouldAutoSelect=true` 才应自动采用 `bestMatch`；`false` 时必须展示候选或保留
人工确认。候选第一名、`confidence` 或高 `matchScore` 都不能代替这个授权字段。
