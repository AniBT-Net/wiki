...
<TypeTable
  type={{
    ...
    'animeId': {
      description: '番剧 ID 值。若同时传 animeIdType，则按所选来源解析；若不传 animeIdType，则保持旧行为，按站内番剧 ID 查找。',
      type: 'string',
    },
    'anime': {
    ...
    'malId': {
      description: '兼容字段：MyAnimeList anime id。等价于 animeIdType=mal + animeId。',
      type: 'number',
    },
    'anidbId': {
      description: '兼容字段：AniDB anime id。等价于 animeIdType=anidb + animeId。',
      type: 'number',
    },
    'anilistId': {
      description: '兼容字段：AniList anime id。等价于 animeIdType=anilist + animeId。',
      type: 'number',
    },
    'title': {
...
| 番剧 ID | 推荐只传一种番剧标识。新接入用 `animeIdType` + `animeId`；旧字段 `bgmId` / `malId` / `anidbId` / `anilistId` 继续兼容。 |
...
| `malId` | MyAnimeList anime id，例如 `52991` | 兼容旧字段。本站按 bangumi-data 的 BGM 对应关系解析。 |
| `anidbId` | AniDB anime id，例如 `17617` | 兼容旧字段。同一个 AniDB ID 对应多个 BGM 条目时会进入待匹配状态。 |
| `anilistId` | AniList anime id，例如 `154587` | 兼容旧字段。同一个 AniList ID 对应多个 BGM 条目时会进入待匹配状态。 |
| 单独 `animeId` | 站内 `anime_...` / Convex doc id 字符串 | 旧高级用法。只有不传 `animeIdType` 时才按站内 ID 解析。 |
...
番剧识别推荐只传一种 ID。`animeIdType + animeId` 或 `anime` 会按指定来源解析；单独 `animeId` 保持旧站内 ID 语义；`bgmId`、`malId`、`anidbId`、`anilistId` 继续兼容。外部 ID 的对应关系以 bangumi-data 为准；若外部 ID 对应多个 BGM 条目、多个外部 ID 解析结果互相冲突，或暂时找不到对应番剧，API Key 发布会进入待匹配状态，之后由字幕组或管理员补匹配。
